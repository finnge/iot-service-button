...
% todo: Überprüfen ob es wirklich ein Interaktionsstil ist. Vllt auch Paradigma ???
...
% ToDo: Abschluss der Interaktion. Warum ende mit nachricht verschicken? warum nicht warten auf success nachricht vom server? (=> MQTT QoS!! vllt doch warten)
...
%\begin{itemize}
%    \item Welche Konzepte wurden verwendet
%    \item Allgemein: Experten-Vorträge / Abwägungen
%\end{itemize}
...
%\begin{itemize}
%    \item Welchen Sinn haben die Sensoren? (verändern Zustände in der Welt / geben interaktionsmöglichkeiten für den nutzer)
%    \item welche alternativen gibt es (licht für taubstumme->ethics)
%    \item Welchen Sensor für Auth (RFID, Fingerabdruck...) \cite{Online:Seedstudio:Groove} \cite{Adryan:TechFound-Components:2017}
%    \item low level communication (I2C etc)
%\end{itemize}
...
% - [X] Warum MQTT? / Gibt es Alternativen?
% - [X] MQTT baut auf TCP/IP auf
% - [X] Im Protoyp wurde AWS genommen (\ref{sec:backend})
% - [X] QoS bei den Nachrichten (-> in Tabelle 2 einfügen)
%   - [X] erste gedanken: alle sind wichtig (keine wetterdaten); order am wichtigsten
% - [ ] LWT: Diskussion ob nötig, theoretisch nur für wartungszwecke
...
\subsection{Sended messages between the components}
...
% - [x] QoS
%   - [x] order messages können nicht mehrmals (order system muss reagieren)
%   - [x] Rest kann Level 1 da mehrmaliges senden keinen unterschied machen soll (+ erklären) 
...
% todo Firmen-Daten werden jedoch gesendet

\section{Discussion and Conclusions}
\label{sec:Future}

%\begin{itemize}
%    \item Affordances need to become signifiers \cite{Norman:EverydayThings:2013} (How do I know that I can interact with something)
%    \item Mögliche Verdeckung des Displays durch Chip-Karte
%\end{itemize}
...
%\begin{itemize}
%    \item Amazon Web Services (abhängigkeit) done
%    \item Datenschutz-Thema: Bestellung mit RFID-Sensor / Fingerabdruck etc (ist das personenbezogen?) done
%   \item ungewollte bestellungen
%    \item Zugänglichkeit (Sound / Licht, Farbkontrast, Sprache, Dauer der anzeige)
%    \item Microcontroller übernimmt keine ethischen Entscheidungen.
%\end{itemize}


% verknüpfung rfid mit order
